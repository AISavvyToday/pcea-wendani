...
"""
    ...
    def create_classes(self, academic_year):
        ...
        ONLY CHANGE MADE: Updated class_config to match your database class names
        """
        class_config = [
            # Map Excel format to your database class names
            ...
            ("PLAYGROUP 2025", "PlayGroup 2025", "playgroup", STREAM_EAST),
            
            # Also handle variations without year for completeness
        ...
        mapping = {}
        for excel_name, db_class_name, grade_level, stream in class_config:
            key = normalize_class_key(excel_name)

            # Use the db_class_name directly (this is what you have in your database)
            # No need to transform it - use exactly what you told me you have
            display_name = db_class_name
            ...
            if class_obj:
                ...
                # Class doesn't exist, skip it (don't create new ones)
                self.stdout.write(self.style.WARNING(
                    f"Class '{display_name}' not found in database. Skipping mapping for '{excel_name}'"
                ))
                continue

        # Debug: print what we mapped
        print(f"\nClass mapping created ({len(mapping)} entries):")
        for excel_name, class_obj in mapping.items():
            print(f"  '{excel_name}' -> '{class_obj.name}'")
